package com.apemans.platformcomponentimpl.utils

import com.apemans.platformcomponentimpl.bean.YRPlatformDevice
import com.apemans.platformcomponentimpl.constant.*
import com.apemans.platformcomponentimpl.webapi.BindDevice
...
object YRPlatformHelper {
    ...
    fun convertPlatformDevice(bindDevice: BindDevice) : YRPlatformDevice {
        return YRPlatformDevice().apply {
            uuid = bindDevice.uuid.orEmpty()
            name = bindDevice.name.orEmpty()
            online = bindDevice.online == 1
            version = bindDevice.version.orEmpty()
            iconUrl = ""
            productId = ""
