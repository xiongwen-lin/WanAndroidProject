...
package com.apemans.quickui.statelayout
...
import com.apemans.quickui.statelayout.Status.*
...
class StateLayout @JvmOverloads constructor(
    ...
    private var errorViewEventIds: IntArray? = null
    private var emptyViewEventIds: IntArray? = null
    ...
    private var onErrorViewEvent: (View.(Any?) -> Unit)? = null
    private var onEmptyViewEvent: (View.(Any?) -> Unit)? = null
    ...
    /**
     * 当[Error]时中绑定的View di 事件会触发回调
     */
    fun setErrorViewEventAction(block: View.(Any?) -> Unit): StateLayout {
        onErrorViewEvent = block
        return this
    }

    /**
     * 当[empty]时中绑定的View di 事件会触发回调
     */
    fun setEmptyViewEventAction(block: View.(Any?) -> Unit): StateLayout {
        onEmptyViewEvent = block
        return this
    }
    ...
    /**
     * 为错误页指定Id控件设置点击事件
     * Error View  中触发的IdRes
     * 会为[StateLayout.errorLayout]中的指定Id的视图对象添加一个点击事件
     * 默认点击500ms内防抖动
     */
    fun setErrorViewEventIds(@IdRes vararg ids: Int) = apply {
        errorViewEventIds = ids
        return@apply
    }
     ...
     * 为空页中的指定Id控件设置点击事件
     * 默认点击500ms内防抖动
     * Empty View  中触发的IdRes
     * 会为[StateLayout.emptyLayout]中的指定Id的视图对象添加一个点击事件
     * 点击500ms内防抖动
     */
    fun setEmptyViewEventIds(@IdRes vararg ids: Int) = apply {
        emptyViewEventIds = ids
        return@apply
    }
    ...
    private fun show(status: Status, tag: Any? = null) {
        ...
        runMain {
            try {
                ...
                when (status) {
                    EMPTY -> {
                        emptyViewEventIds?.forEach {
                            view.findViewById<View>(it)?.apply {
                                throttleClick {
                                    //showLoading()
                                    onEmptyViewEvent?.invoke(this, tag)
                                }
                            }
                        }
                    ...
                    ERROR -> {
                        errorViewEventIds?.forEach {
                            view.findViewById<View>(it)?.throttleClick {
                                //showLoading()
                                onErrorViewEvent?.invoke(this, tag)
                            }
                        }
