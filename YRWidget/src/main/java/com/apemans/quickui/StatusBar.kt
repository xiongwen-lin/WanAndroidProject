...
/**
 * Copy from:https://github.com/liangjingkanji/StatusBar
 * Edit by caro
 * 半透明状态栏
...
fun Activity.immersive(@ColorInt color: Int = COLOR_TRANSPARENT, darkMode: Boolean? = null) {
    when {
        ...
        Build.VERSION.SDK_INT > 21 -> {
            when (color) {
                COLOR_TRANSPARENT -> {
                    //FLAG_TRANSLUCENT_STATUS Deprecated in API level 30
                    window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS)
 ...
 *
 * @param darkMode 状态栏文字是否为暗色 默认为黑色
 */
fun Activity.darkMode(darkMode: Boolean = true) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        ...
        Log.i("StatusBar","Activity.darkMode @darkMode=$darkMode | systemUiVisibility:$systemUiVisibility")
        window.decorView.systemUiVisibility = systemUiVisibility
    }
}
