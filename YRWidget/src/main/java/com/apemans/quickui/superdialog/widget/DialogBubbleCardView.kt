package com.apemans.quickui.superdialog.widget
...
class DialogBubbleCardView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : View(context, attrs, defStyleAttr) {

    var a = context.obtainStyledAttributes(attrs, R.styleable.DialogBubbleCardView, defStyleAttr, 0)
    private val backgroundColor = a.getColor(
        R.styleable.DialogBubbleCardView_bubble_dialog_backgroundColor, Color.WHITE
    )

    // 0 1 2 3 = t b l r
    var orientation = a.getInteger(R.styleable.DialogBubbleCardView_bubble_triangleOrientation, OR_BOTTOM)
    ...
    override fun layout(l: Int, t: Int, r: Int, b: Int) {
        ...
        sizeRF.set(roundRadius, roundRadius, (r - l) - roundRadius, (b - t) - roundRadius)
    ...
    fun setSubscriptCenterX(x: Float) {
        val sx = min(max(x, roundRadius + subscriptWidth + roundRadius), sizeRFSource.width() - roundRadius)
        updateSizeRF()
        path.reset()
    ...
    private fun setSubscriptCenterY(y: Float) {
        val sy = min(max(y, roundRadius + subscriptWidth + roundRadius), sizeRFSource.height() - roundRadius)
        updateSizeRF()
        path.reset()
    ...
    fun setBubbleOutset(v: Int) {
        if (orientation in 0..1) {
            setSubscriptCenterX(v + roundRadius)
        } else {
            setSubscriptCenterY(v.toFloat() + roundRadius)
        }
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        canvas.drawRoundRect(sizeRF, roundRadius, roundRadius, paint)
