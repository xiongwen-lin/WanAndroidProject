...
import com.apemans.usercomponent.baseinfo.bean.Country;
import com.apemans.usercomponent.baseinfo.bean.CountryViewBean;
...
public class CountryUtil {
    public static final String DEFAULT_COUNTRY_KEY = "US";
    ...
    public static final HashMap<String, String> mIdWithJaHashMap = new HashMap();
    ...
    public static void putCountryDataToMap(ArrayList<Country> countryDatas) {
        int size = countryDatas.size();

        for(int i = 0; i < size; ++i) {
            Country country = (Country)countryDatas.get(i);
            String key = country.getAbbr();
            mIdWithChiHashMap.put(key, country.getChinese());
            mIdWithEngHashMap.put(key, country.getEnglish());
            mIdWithFrHashMap.put(key, country.getFr());
            mIdWithItHashMap.put(key, country.getIt());
            mIdWithDeHashMap.put(key, country.getDe());
            mIdWithEsHashMap.put(key, country.getEs());
            mIdWithJaHashMap.put(key, country.getJa());
    ...
    public static List<ContactItemInterface> getSampleContactList(Context context) {
        if (!isPutInMap) {
            isPutInMap = true;
            putCountryDataToMap(getDefaultCountryData());
        }

        List<ContactItemInterface> list = new ArrayList();
        Iterator iter = getCountryMap(context).entrySet().iterator();

        while(iter.hasNext()) {
            Map.Entry<String, String> entry = (Map.Entry)iter.next();
            String key = (String)entry.getKey();
            String val = (String)entry.getValue();
            list.add((ContactItemInterface) new CountryViewBean(key, val, (String)mIdWithNumHashMap.get(key), (String)mIdWithPinHashMap.get(key), context));
        }

        return list;
    }

    public static HashMap<String, String> getCountryMap(Context context) {
        if (!isPutInMap) {
            isPutInMap = true;
            putCountryDataToMap(getDefaultCountryData());
        }

        return FontUtil.isZh(context) ? mIdWithChiHashMap : mIdWithEngHashMap;
    }
    ...
    public static String getCountryNum(String key) {
        ...
        return TextUtils.isEmpty(key) ? "" : (String)mIdWithNumHashMap.get(key);
    }

    public static String getCountryTitle(Context context, String key) {
        if (!isPutInMap) {
            isPutInMap = true;
            putCountryDataToMap(getDefaultCountryData());
        }
        ...
        if (TextUtils.isEmpty(key)) {
            ...
            return FontUtil.isZh(context) ? (String)mIdWithChiHashMap.get(key) : (String)mIdWithEngHashMap.get(key);
        }
    }

    public static String getCountryKey(Context context) {
        String countryKey = "US";

        try {
            @SuppressLint("WrongConstant") TelephonyManager tm = (TelephonyManager)context.getSystemService("phone");
            countryKey = !TextUtils.isEmpty(tm.getNetworkCountryIso()) ? tm.getNetworkCountryIso().toUpperCase() : Locale.getDefault().getCountry();
    ...
    public static String getCurrentCountryKey(Context context) {
        String countryKey = TextUtils.isEmpty(getCountryKey(context)) ? "US" : getCountryKey(context);
        return countryKey;
    }

    public static String getCurrentCountry(Context context) {
        String countryKey = TextUtils.isEmpty(getCountryKey(context)) ? "US" : getCountryKey(context);
        return getCountryNum(countryKey);
    }

    public static String getCurrentCountryTitle(Context context) {
        String countryKey = TextUtils.isEmpty(getCountryKey(context)) ? "US" : getCountryKey(context);
        return getCountryTitle(context, countryKey);
    }
    ...
    public static String getTimeZoneByRawOffset(int rawOffset) {
        ...
        timeDisplay = timeDisplay + String.format(Locale.getDefault(), "%02d:%02d", hour, minute);
        return timeDisplay;
    }
    ...
    public static String getTimeZone() {
        try {
            ...
            return !TextUtils.isEmpty(str) ? str.substring(0, 3) + ":" + str.substring(3) : defaultZone();
    ...
    public static String getTimezoneGCMById(String timezoneId) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            timeZoneByRawOffset = timeZone.getRawOffset();
        } else {
    ...
    public static int getTimezoneCodeById(String timezoneId) {
        int timeZoneByRawOffset;
        if (Build.VERSION.SDK_INT >= 24) {
            android.icu.util.TimeZone timeZone = android.icu.util.TimeZone.getTimeZone(timezoneId);
            timeZoneByRawOffset = timeZone.getRawOffset();
        ...
        int hour = timeZoneByRawOffset / 1000 / 3600;
        return hour;
    }
    ...
    public static String getCurrentTimezoneGCM() {
        return getTimezoneGCMById(getTimeZoneId());
    }

    public static String getTimeZoneByRawOffsetForCmd(int rawOffset) {
        String timeDisplay = rawOffset >= 0 ? "+" : "-";
        int hour = rawOffset / 1000 / 3600;
        int minute = (rawOffset - hour * 1000 * 3600) / 1000 / 60;
        timeDisplay = timeDisplay + String.format(Locale.getDefault(), "%02d.%02d", Math.abs(hour), minute);
        return timeDisplay;
    }

    public static String getTimezoneGCMByIdForCmd(String timezoneId) {
        int timeZoneByRawOffset;
        if (Build.VERSION.SDK_INT >= 24) {
            android.icu.util.TimeZone timeZone = android.icu.util.TimeZone.getTimeZone(timezoneId);
            timeZoneByRawOffset = timeZone.getRawOffset();
        ...
        return getTimeZoneByRawOffsetForCmd(timeZoneByRawOffset);
    }

    public static String getCurrentTimezoneGCMForCmd() {
        return getTimezoneGCMByIdForCmd(getTimeZoneId());
    }

    public static boolean isDaylight(TimeZone zone, Date date) {
        if (zone != null && date != null) {
