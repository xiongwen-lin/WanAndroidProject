...
class BleCore(context: Context) : BaseBleManager(context) {
    ...
    private inner class GattCallback : BleManagerGattCallback() {
        ...
        override fun isRequiredServiceSupported(gatt: BluetoothGatt): Boolean {
            ...
            if (service == null) {
                BleLogger.i(TAG, "isRequiredServiceSupported --> service uuid is null 获取不到service")
                //找不到服务特征时，将断开当前的连接
                disconnectBle()
                return false
            }
...
//                writeRequest =
//                    (rxProperties and BluetoothGattCharacteristic.PROPERTY_WRITE > 0) || (rxProperties and PROPERTY_WRITE_NO_RESPONSE > 0)
//
...
//            }
            ...
            if (!UUID_READ_CHAR.isNullOrEmpty() && UUID_NOTIFY_CHAR.isNullOrEmpty()) {
                isSupport = mReadCharacteristic != null && writeRequest
            }

            if (UUID_READ_CHAR.isNullOrEmpty() && !UUID_NOTIFY_CHAR.isNullOrEmpty()) {
                isSupport = mNotifyCharacteristic != null && writeRequest
            }

            if (!UUID_READ_CHAR.isNullOrEmpty() && !UUID_NOTIFY_CHAR.isNullOrEmpty()) {
                isSupport =
                    mNotifyCharacteristic != null && mReadCharacteristic != null && writeRequest
            }
    ...
    private val scanCallback: ScanCallback = object : ScanCallback() {
        ...
        override fun onBatchScanResults(results: List<ScanResult>) {
            //BleLogger.i(TAG, "ScanCallback --> onBatchScanResults --- >$results")
            //每回调一次就情况一次
            //scanResultFilterList.clear()
            for (i in results.indices) {
                ...
                if (!filterScanResultFullName.isNullOrEmpty() && devName == filterScanResultFullName) {
