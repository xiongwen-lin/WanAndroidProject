package com.apemans.bluetooth.observer
...
class NordicBleConnectionObserver(private val connectionCallbackList: List<ConnectionCallback>) :
    ...
    override fun onDeviceConnecting(device: BluetoothDevice) {
        val isAutoReConnect = SmartBleManager.core.autoReConnectWhenOnLinkLoss
        BleLogger.i(TAG, "onDeviceConnecting设备正在连接 ，是否开启了重新连接 -- isAutoReConnect：$isAutoReConnect | isLinkLoss:$isLinkLoss")
        if (!isLinkLoss) {
            //如果不是Link Loss （开启了isAutoReConnect）的原因，才回调连接状态
